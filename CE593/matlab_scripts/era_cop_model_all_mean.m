...
mean_dur = 12;   % last x hours mean wind
...
for j = 1:N
    ...
    U_eff(j)   = hypot(u_bar,v_bar);
    dir_eff(j) = mod(atan2d(-u_bar,-v_bar),360);
end

% =========================
% READ ERA5 WIND WAVES
% =========================
...
for j = 1:N-1
    ...
    if U_eff(j+1)>0
        Hmo_star(j+1) = g*Hmo_model(j)/(U_eff(j+1)^2);
    else
        Hmo_star(j+1) = 0;
    end

    t_prime_star = 368000 * Hmo_star(j+1)^(4/3);
    t_prime      = t_prime_star * U_eff(j+1) / g;
