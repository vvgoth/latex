...
time_era = datetime(1970,1,1) + seconds( ...
           ncread(filename_era_tm,'valid_time'));

Hs_cop = squeeze(ncread(filename_cop,'VHM0_WW'));
Tm_cop = squeeze(ncread(filename_cop,'VTM01_WW'));
time_cop = datetime(1970,1,1) + seconds( ...
           ncread(filename_cop,'time'));
...
for s = 1:nStorms
    Hs_mod_all = [Hs_mod_all; Storms(s).Hs_model];
    Hs_era_all = [Hs_era_all; Storms(s).Hs_era];
    Hs_cop_all = [Hs_cop_all; Storms(s).Hs_cop];

    Tm_mod_all = [Tm_mod_all; Storms(s).Tm_model];
    Tm_era_all = [Tm_era_all; Storms(s).Tm_era];
    Tm_cop_all = [Tm_cop_all; Storms(s).Tm_cop];
end
...
for k = 1:length(top3)
    ...
    grid on
    title(sprintf('Storm %d – H_s comparison',s))
    ylabel('H_s (m)')
    xlabel('Time')
    ...
    grid on
    title(sprintf('Storm %d – T_m comparison',s))
    ylabel('T_m (s)')
    xlabel('Time')
    legend('Model','ERA5','Copernicus','Location','northwest')
end
...
function [H_mo, T_p] = jonswap_basic(fetch,U_input,t_duration)
    ...
    H_mo_a = 0.243*U_input.^2/g;
    T_p_a  = 8.13*U_input/g;

    F_eff = (t_star/68.8).^(3/2);
    F_use = min(F_star,F_eff);

    H_mo_b = 0.0016*F_use.^(1/2).*U_input.^2/g;
    T_p_b  = 0.286 *F_use.^(1/3).*U_input/g;
