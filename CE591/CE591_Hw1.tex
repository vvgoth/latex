...
\section{Question 1} Select a coastal engineering problem (in regional scale), and discuss which governing equations can be used to model the waves for your selected problem. Explain the reason why you have selected these equations with a few paragraphs.

\vspace{0.3cm}A classic coastal engineering problem is wave transformation from offshore toward the coastline. In this type of problem, waves travel over a large area and change due to depth variation, coastal shape, and energy losses. The main processes include shoaling, refraction, and wave breaking near the shore.

Several governing equations can be used depending on the scale and purpose of the study. Linear wave theory can be used for basic wave properties such as wave speed, wavelength, and particle motion. Shallow water equations can be used when water depth is small compared to wavelength and when wave-driven water level changes or currents are important. Spectral wave models are also commonly used for regional problems because they are suitable for irregular sea states and large coastal areas. On the other hand, the Navier-Stokes equations give a much more detailed description of fluid motion, but they are generally used for small-scale problems because solving them over a large coastal region would require very high computational effort.

For regional wave transformation problems, simplified wave equations or spectral wave models are usually preferred over the full Navier-Stokes equations. This is because they can represent the main coastal wave processes with much lower computational cost while still giving results that are suitable for engineering applications.
...
The plot in Figure 1 shows the surface elevation of a Stokes' second-order wave at x = 0 and t = 0-8s. The surface elevation is calculated using the following equations:
...
The vertical distributions of horizontal and vertical particle velocities were obtained using both linear theory and Stokes' second-order theory. Since the velocity values depend on phase, the horizontal velocity was plotted at $\theta = 0$, where its magnitude is maximum, while the vertical velocity was plotted at $\theta = \pi/4$ so that both first- and second-order contributions are visible.

\[
u_1(z,t)=\frac{H\omega}{2}\frac{\cosh[k(h+z)]}{\sinh(kh)}\cos\theta
\]
\[
u_2(z,t)=\frac{3}{16}c(kH)^2
\frac{\cosh[2k(h+z)]}{\sinh^4(kh)}
\cos(2\theta)
\]
\[u = u_1 + u_2\]

\[
w_1(z,t)=-\frac{H\omega}{2}\frac{\sinh[k(h+z)]}{\sinh(kh)}\sin\theta
\]
\[
w_2(z,t)=-\frac{3}{16}c(kH)^2
\frac{\sinh[2k(h+z)]}{\sinh^4(kh)}
\sin(2\theta)
\]
\[w = w_1 + w_2\]
...
The results show that the horizontal velocity is largest near the free surface and decreases with depth, while the vertical velocity also becomes stronger toward the surface and approaches zero near the seabed. This behavior is consistent with wave kinematics, since orbital motion weakens with depth. Compared with linear theory, the second-order Stokes solution predicts slightly different velocity magnitudes, especially close to the free surface where nonlinear effects are stronger.

Overall, the two theories give similar trends, but the second-order solution provides a more realistic description by including nonlinear corrections. These corrections do not completely change the shape of the velocity profiles, but they slightly modify the near-surface values and therefore improve the physical representation of the wave motion.
\vspace{0.3cm}
...
\begin{itemize}  
    \item {Shallower depth (\( h = 20 \, \text{m} \))}: The difference between linear theory and Stokes second-order theory becomes more noticeable. In the horizontal velocity plot, the Stokes second-order values are lower than the linear values near the surface. A similar trend appears in the vertical velocity plot, where the second-order profile also departs more clearly from the linear one. This indicates that nonlinear effects become relatively stronger as the depth decreases.
    \begin{figure}[H]
        \centering
...
The results show that the pressure decays more rapidly with depth in deeper water. Therefore, for h=30 m, the non-dimensional dynamic pressure becomes smaller at a given relative depth than in the h=20 m case. In shallower water, the decay is weaker, so a larger portion of the surface pressure signal reaches the lower part of the water column. This behavior is physically consistent, since wave-induced pressure fluctuations penetrate more effectively through shallow water than through deeper water.
...
The theoretical solution produces a single symmetric crest with amplitude equal to the wave height. When plotted against time at a fixed location, the curve rises to a maximum and then drops smoothly on both sides. In general, some difference between the two curves is expected because of laboratory effects such as measurement noise, viscous dissipation, and imperfect wave generation. However, since the overall crest shape and peak elevation are similar, the analytical solution can be considered a reasonable representation of the measured wave.
